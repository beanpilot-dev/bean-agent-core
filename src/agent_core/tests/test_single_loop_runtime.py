...
def test_model_tool_manifest_excludes_execution_tools() -> None:
    ...
    assert "ledger_prepare_balance_reconciliation" in model_names
    assert "ledger_prepare_reconciliation" not in model_names
...
def test_ledger_prepare_change_set_returns_approval_required_without_write(
    ...
    assert payload["pending_action"]["status"] == "PENDING_ACTION"
    assert (
        payload["pending_action"]["execution_spec"]["commit_message"] == "record savings transfer"
    )
...
def test_ledger_prepare_balance_reconciliation_returns_approval_required_without_write(
    ledger_workspace: Path,
) -> None:
    target = ledger_workspace / "data" / "agent_inc" / f"{date.today():%Y-%m}.beancount"
    original = target.read_text()

    raw = tool_ledger_prepare_balance_reconciliation.func(
        "2026-05-31",
    ...
    assert payload["status"] == "approval_required"
    assert payload["tool_name"] == "ledger_prepare_balance_reconciliation"
